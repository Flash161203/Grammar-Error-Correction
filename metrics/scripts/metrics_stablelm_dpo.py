...
def preprocess_pretrained_model(examples):
    ...
    strinputs = tokenizer.batch_decode(model_inputs["input_ids"])
    prompts = [get_prompt(text, tokenizer) for text in strinputs]
    return {"prompts": prompts, "labels": labels_out, "inputs": strinputs}
...
def run_pipeline_and_evaluate():
    ...
    print(f"model loaded: {dpo_adapter}")
    remove_columns = [
    ...
    with open("stablelm_dpo_data.json", "w") as f:
        json.dump(data, f)

    metrics = {"bleu": bleu_score, "rouge": rouge_score, "bertscore": bert_score}
    with open("stablelm_dpo_metrics.txt", "w") as f:
