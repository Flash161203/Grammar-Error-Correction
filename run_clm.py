...
def main():
    ...
    def tokenize_function(examples):
        with CaptureLogger(tok_logger) as cl:
            output = tokenizer(
                examples[text_column_name], truncation=True, max_length=2048
            )
    ...
    with training_args.main_process_first(desc="dataset map tokenization"):
        if not data_args.streaming:
            ...
            raw_datasets = raw_datasets.map(
                make_correct_sentence_column,
                num_proc=data_args.preprocessing_num_workers,
                remove_columns=["id", "userid", "cefr", "edits"],
            )
            print(raw_datasets.column_names)
            ...
            raw_datasets = raw_datasets.map(
                create_prompt_formats, remove_columns=["text", "correct_text"]
            )
            tokenized_datasets = raw_datasets.map(
